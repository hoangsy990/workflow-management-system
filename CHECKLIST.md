...
**Tiến độ hiện tại:** `DONE=136`, `PARTIAL=125`, `TODO=0`, `WAITING=0`, `BLOCKED=0`, `TOTAL=261`; hoàn thành nghiêm ngặt `52.1%`, tính trọng số partial `76.1%`.
...
| Danh mục tùy chỉnh có fields/status/scope/manager | `PARTIAL` | Có DB/migration/API nền tảng `shared_catalogs`, `shared_catalog_fields`, `shared_catalog_items` với status, scope department, manager, fields, item values, seed mẫu, audit; UI tạo/sửa/xóa catalog + item, picker phạm vi phòng ban/quản lý, search và export CSV đã có smoke Docker. Còn thiếu import CSV/XLSX catalog có preview/transaction. |
